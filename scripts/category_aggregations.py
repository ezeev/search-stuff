...
'''
CATEGORY:
SELECT cleaned_query_s, category_s, type_s, count(*) as event_count, avg(click_time_i) as avg_ts FROM acme_events GROUP BY cleaned_query_s, category_s, type_s HAVING count(*) > 10 ORDER BY count(*) DESC
'''
...
delete_url = delete_url = 'http://localhost:8983/solr/bb_clicks_aggr/update?stream.body=<delete><query>aggr_type_s:category</query></delete>'
...
while cur_click_time <= max_click_time + seconds_in_day:
    ...
    aggr_sql = 'SELECT query_s, category_s, type_s, count(*) as event_count, avg(CAST( click_time_i AS double )) as avg_ts FROM bb_clicks WHERE click_time_i BETWEEN %d AND %d GROUP BY query_s, category_s, type_s HAVING count(*) > 2 ORDER BY count(*) DESC LIMIT 1000000' % (cur_click_time, upper_click_time)
    ...
    for doc in data['result-set']['docs']:
        if 'query_s' in doc:
            new_doc = {}
            new_doc['query_txt_en'] = doc['query_s']
            new_doc['query_s'] = doc['query_s'].lower()
            new_doc['aggr_type_s'] = "category"
            new_doc['type_s'] = doc['type_s']
            new_doc['category_s'] = doc['category_s']
